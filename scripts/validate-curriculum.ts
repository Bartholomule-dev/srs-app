...
// Validates exercise metadata against curriculum definition
// Converted from validate-exercises.cjs - checks curriculum consistency, not schema
...
function validateCurriculum() {
  ...
  const curriculum: CurriculumGraph = JSON.parse(readFileSync(curriculumPath, 'utf-8'));
  ...
  const subconceptToConcept = new Map<string, string>();
  ...
  // Load all exercises
  const exerciseDir = join(process.cwd(), 'exercises/python');
  const exerciseFiles = readdirSync(exerciseDir).filter(f => f.endsWith('.yaml'));
  const allExercises = new Map<string, YamlExercise>();
  const exercisesBySubconcept = new Map<string, YamlExercise[]>();

  for (const file of exerciseFiles) {
    const content = readFileSync(join(exerciseDir, file), 'utf8');
